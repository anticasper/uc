...
final class Api_No_Parque {
    ...
    private function __construct() {
        ...
        add_action('save_post_uc', array($this, 'save_uc_quick_info'), 22, 2);
    ...
    public function render_uc_admin_form($post) {
        ...
        $estacionamento_status = get_post_meta($post->ID, '_uc_estacionamento_status', true);
        $estacionamento_tipo = get_post_meta($post->ID, '_uc_estacionamento_tipo', true);
        $acessibilidade_status = get_post_meta($post->ID, '_uc_acessibilidade_status', true);
        ...
        <div class="api-np-uc-form" data-api-np-uc-form>
            ...
            <section class="api-np-uc-section">
                <div class="api-np-uc-section__header">
                    <h2><?php esc_html_e('Informacoes rapidas', 'api-no-parque'); ?></h2>
                </div>

                <div class="api-np-uc-grid api-np-uc-grid--4">
                    <label class="api-np-uc-field" for="uc_estacionamento_status">
                        <span><?php esc_html_e('Area para estacionamento', 'api-no-parque'); ?></span>
                        <select id="uc_estacionamento_status" name="uc_estacionamento_status">
                            <option value=""><?php esc_html_e('Nao informado', 'api-no-parque'); ?></option>
                            <option value="sim" <?php selected($estacionamento_status, 'sim'); ?>><?php esc_html_e('Sim', 'api-no-parque'); ?></option>
                            <option value="nao" <?php selected($estacionamento_status, 'nao'); ?>><?php esc_html_e('Nao', 'api-no-parque'); ?></option>
                        </select>
                    </label>

                    <label class="api-np-uc-field" for="uc_estacionamento_tipo">
                        <span><?php esc_html_e('Tipo de estacionamento', 'api-no-parque'); ?></span>
                        <select id="uc_estacionamento_tipo" name="uc_estacionamento_tipo">
                            <option value=""><?php esc_html_e('Nao informado', 'api-no-parque'); ?></option>
                            <option value="gratuito" <?php selected($estacionamento_tipo, 'gratuito'); ?>><?php esc_html_e('Gratuito', 'api-no-parque'); ?></option>
                            <option value="pago" <?php selected($estacionamento_tipo, 'pago'); ?>><?php esc_html_e('Pago', 'api-no-parque'); ?></option>
                        </select>
                    </label>

                    <label class="api-np-uc-field" for="uc_acessibilidade_status">
                        <span><?php esc_html_e('Acessibilidade', 'api-no-parque'); ?></span>
                        <select id="uc_acessibilidade_status" name="uc_acessibilidade_status">
                            <option value=""><?php esc_html_e('Nao informado', 'api-no-parque'); ?></option>
                            <option value="sim" <?php selected($acessibilidade_status, 'sim'); ?>><?php esc_html_e('Sim', 'api-no-parque'); ?></option>
                            <option value="nao" <?php selected($acessibilidade_status, 'nao'); ?>><?php esc_html_e('Nao', 'api-no-parque'); ?></option>
                        </select>
                    </label>
                </div>
            </section>
    ...
    public function save_uc_quick_info($post_id, $post) {
        if (!isset($_POST['uc_dados_nonce_field']) || !wp_verify_nonce(sanitize_key($_POST['uc_dados_nonce_field']), 'uc_dados_nonce')) {
            return;
        }
        ...
        $fields = array(
            '_uc_estacionamento_status' => array(
                'request_key' => 'uc_estacionamento_status',
                'allowed' => array('sim', 'nao'),
            ),
            '_uc_estacionamento_tipo' => array(
                'request_key' => 'uc_estacionamento_tipo',
                'allowed' => array('gratuito', 'pago'),
            ),
            '_uc_acessibilidade_status' => array(
                'request_key' => 'uc_acessibilidade_status',
                'allowed' => array('sim', 'nao'),
            ),
        );

        foreach ($fields as $meta_key => $settings) {
            $value = isset($_POST[$settings['request_key']]) ? sanitize_key(wp_unslash($_POST[$settings['request_key']])) : '';

            if ($value && in_array($value, $settings['allowed'], true)) {
    ...
    private function update_uc_meta(int $post_id, array $item) {
        $meta_map = array(
            ...
            '_uc_social' => 'social',
            '_uc_estacionamento_status' => 'estacionamento_status',
            '_uc_estacionamento_tipo' => 'estacionamento_tipo',
            '_uc_acessibilidade_status' => 'acessibilidade_status',
        );
    ...
    private function get_uc_parking_label(int $post_id) {
        $status = (string) get_post_meta($post_id, '_uc_estacionamento_status', true);
        $type = (string) get_post_meta($post_id, '_uc_estacionamento_tipo', true);

        if ('nao' === $status) {
            return 'Nao possui area para estacionamento';
        }
        ...
        if ('gratuito' === $type) {
            return 'Possui estacionamento gratuito';
        }

        if ('pago' === $type) {
            return 'Possui estacionamento pago';
        }

        return 'Possui area para estacionamento';
    }

    private function get_uc_accessibility_label(int $post_id) {
        $status = (string) get_post_meta($post_id, '_uc_acessibilidade_status', true);

        if ('sim' === $status) {
            return 'Possui acessibilidade para pessoas com deficiencia';
        }

        if ('nao' === $status) {
            return 'Nao possui acessibilidade informada';
        }
    ...
    private function format_uc_item(int $post_id) {
        ...
        $estacionamento_status = (string) get_post_meta($post_id, '_uc_estacionamento_status', true);
        $estacionamento_tipo = (string) get_post_meta($post_id, '_uc_estacionamento_tipo', true);
        $acessibilidade_status = (string) get_post_meta($post_id, '_uc_acessibilidade_status', true);
        $estacionamento_label = $this->get_uc_parking_label($post_id);
        $acessibilidade_label = $this->get_uc_accessibility_label($post_id);
        ...
        return array(
            ...
            'estacionamento_status' => $estacionamento_status,
            'estacionamento_tipo' => $estacionamento_tipo,
            'estacionamento' => $estacionamento_label,
            'estacionamento_text' => $estacionamento_label,
            'acessibilidade_status' => $acessibilidade_status,
            'acessibilidade' => $acessibilidade_label,
            'acessibilidade_text' => $acessibilidade_label,
