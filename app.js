...
const stateGroupTemplate = document.querySelector("#state-group-template");
const parkItemTemplate = document.querySelector("#park-item-template");
const form = document.querySelector("#filters-form");
const searchInput = document.querySelector("#search");
const stateSearchInput = document.querySelector("#state-search");
...
function buildPopup(item) {
  ...
  const details = [
    item.biome ? `<span>${item.biome}</span>` : "",
  ...
  return `
    <div class="map-popup">
      ...
      <h3 class="map-popup__title">${item.name}</h3>
      ${details ? `<p class="map-popup__details">${details}</p>` : ""}
      <p class="map-popup__text">${item.description || item.endereco || "Sem descricao."}</p>
    </div>
  `;
}
...
function updateActiveCard() {
  document.querySelectorAll(".result-card").forEach((card) => {
    card.classList.toggle("is-active", Number(card.dataset.id) === state.activeId);
  });
}
...
function applyPins(nextPins) {
  allParks = nextPins
    ...
    .map((item) => ({
      id: item.id,
      name: item.name ?? "Local sem nome",
      state: item.state ?? "--",
      city: item.city ?? "Cidade",
      biome: item.biome ?? "Bioma",
      ...
      atividades: Array.isArray(item.atividades) ? item.atividades : [],
...
async function loadMapData() {
  ...
  setStatus("Carregando os locais reais da base...");

  try {
    const response = await fetch("./map-data.json", { cache: "no-store" });

    if (!response.ok) {
      throw new Error(`Falha ao carregar map-data.json (${response.status})`);
    }
    ...
    resultsCount.textContent = "0";
    dialogCount.textContent = "0";
    setStatus("Nao foi possivel carregar os pins do mapa. Abra este projeto em um servidor local para liberar o fetch do JSON.", "error");
  }
}
...
["park-filter", "biome-filter", "city-filter", "activity-filter"].forEach((id) => {
  document.getElementById(id).addEventListener("change", filterItems);
});
